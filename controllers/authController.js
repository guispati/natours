...
const createSendToken = (user, statusCode, res) => {
    const tokenExpirationInMiliseconds = new Date(Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000);
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(new AppError('There is no user with the entered email address', 404));
    }
